...
syms R R_w real             % 轮半径, 左右半轮距(左轮与右轮之间距离一半)

% ---------- 机体参数 ----------
syms m_b      real   % 机体质量 (kg)
syms I_b      real   % 机体绕质心俯仰转动惯量 (kg·m²)
syms l_body_com real % 机体质心到髋关节的距离 (m)
syms theta_b0 real   % 机体结构质心偏置角 (rad)：在机体坐标系中，质心偏离正下方的角度
syms l_tail_mount_h real % 尾电机安装点相对机体质心的前向偏移 (m)（机头方向为正）
syms l_tail_mount_v real % 尾电机安装点相对机体质心的下向偏移 (m)（下方为正）

% ---------- 腿部参数 ----------
syms m_l      real   % 左腿质量 (kg)
syms m_r      real   % 右腿质量 (kg)
syms I_l      real   % 左腿绕髋关节转动惯量 (kg·m²)
syms I_r      real   % 右腿绕髋关节转动惯量 (kg·m²)
syms l_l      real   % 左腿长度，髋关节到轮轴距离 (m)
syms l_r      real   % 右腿长度，髋关节到轮轴距离 (m)
syms l_leg_l_com real % 左腿质心到轮轴的距离 (m)
syms l_leg_r_com real % 右腿质心到轮轴的距离 (m)
syms theta_l0 real   % 左腿质心偏置角 (rad)：腿质心偏离腿杆方向的固定角度,0度是质心在髋正下方
syms theta_r0 real   % 右腿质心偏置角 (rad)：腿质心偏离腿杆方向的固定角度

% ---------- 轮子参数 ----------
syms m_wl     real   % 左轮质量 (kg)
syms m_wr     real   % 右轮质量 (kg)
syms I_wl     real   % 左轮绕轮轴转动惯量 (kg·m²)
syms I_wr     real   % 右轮绕轮轴转动惯量 (kg·m²)

% ---------- 尾巴参数 ----------
syms m_t      real   % 尾巴质量 (kg)
syms I_t      real   % 尾巴绕尾电机轴转动惯量 (kg·m²)
syms l_tail_com real  % 尾电机轴到尾巴质心的距离 (m)
syms delta_t  real   % 尾巴质心偏置角 (rad)：质心偏离尾杆方向的固定角度，0度是质心在连接点正右方

% ---------- Yaw转动惯量 ----------
syms I_yaw    real   % 整机绕竖直轴（偏航轴）的等效转动惯量 (kg·m²)


% ====== 广义坐标 q ======
% q = [X_b_h, phi, theta_l, theta_r, theta_b, theta_t]
syms X_b_h    phi    theta_l    theta_r    theta_b    theta_t     real
%X_b_h向右为前进正方向，phi上视图逆时针为正方向，theta_l右视图顺时针为正方，theta_r右视图顺时针为正方向，theta_b 右视图逆时针为正方向，theta_t右视图逆时针为正方向
% ====== 广义速度 dq ======
...
%T_hip_r T_hip_l施加机身逆时针为正，T_motor_wr T_motor_wl施加腿逆时针为正，T_tail施加机身逆时针为正
%%
% 将向量组织为列向量，方便后续矩阵运算
q   = [X_b_h;    phi;    theta_l;    theta_r;    theta_b;    theta_t   ];
...
K_legl = 1/2 * m_l * (v_hip_h^2 + v_hip_v^2) + 1/2 * I_l * dtheta_l^2;
K_legr = 1/2 * m_r * (v_hip_h^2 + v_hip_v^2) + 1/2 * I_r * dtheta_r^2;

K_body = 1/2 * m_b * (v_hip_h^2 + v_hip_v^2) + 1/2 * I_b * dtheta_b^2;
...
K_tail = 1/2 * m_t * (v_mount_h^2 + v_mount_v^2) + 1/2 * I_t * (dtheta_t + dtheta_b)^2;
...
% 腿质心
%V_legl = m_l * g * ( y_hip - l_l*cos(theta_l) + l_leg_l_com*cos(theta_l+theta_l0));
%V_legr = m_r * g * ( y_hip - l_r*cos(theta_r) + l_leg_r_com*cos(theta_r+theta_r0));
V_legl = R+l_leg_l_com*cos(theta_l+theta_l0);
V_legr = R+l_leg_r_com*cos(theta_r+theta_r0);
% 机体质心
...
% Q_{phi}: 左右轮力矩差产生偏航力矩
Q(2) = R_w / R * (T_motor_wr - T_motor_wl);
