...
% ---------- 尾巴参数 ----------
% l_tail_mount_h：安装点相对机体质心前向距离
% l_tail_mount_v：安装点相对机体质心下向距离的绝对值
% l_tail_com：尾电机安装点到尾巴质心距离
...
syms F_t_to_b_h F_t_to_b_v T_t_to_b real    % 尾巴对机身反作用力/力矩
...
eq_tail_rot = I_t * ddtheta_t + T_t_to_b - m_t * g * l_tail_com * cos(alpha_t);

% 左腿方程 (3个)
eq_leg_l_h = -F_l_to_b_h + F_wl_to_l_h - m_l*a_l_h;
eq_leg_l_v = -F_l_to_b_v + F_wl_to_l_v + m_l*g - m_l*a_l_v;
eq_leg_l_rot = -T_l_to_b + T_wl_to_l + m_l*g*l_leg_l_com*sin(theta_l + theta_l0) ...
...
eq_leg_r_rot = -T_r_to_b + T_wr_to_r + m_r*g*l_leg_r_com*sin(theta_r + theta_r0) ...
...
% 最终方程3: 右腿转动方程
eq3 = I_r*ddtheta_r - T_wr_to_r + T_r_to_b - m_r*g*l_leg_r_com*sin(theta_r + theta_r0) ...
...
% 最终方程4: 左腿转动方程
eq4 = I_l*ddtheta_l - T_wl_to_l + T_l_to_b - m_l*g*l_leg_l_com*sin(theta_l + theta_l0) ...
...
% 最终方程6: 尾巴转动方程
eq6 = I_t*ddtheta_t + T_t_to_b - m_t*g*l_tail_com*cos(alpha_t);
...
disp(eq2);
fprintf('\n推导文档 §3.2 参考形式:\n');
fprintf('  I_b*ddtheta_b = T_l + T_r + m_b*g*l_body_com*sin(theta_b + theta_b0)\n\n');
...
disp(eq3);
fprintf('\n推导文档 §3.3 参考形式:\n');
fprintf('  I_r*ddtheta_r = T_wr - T_r + m_r*g*l_leg_r_com*sin(theta_r + theta_r0)\n');
...
% 构建参考方程
eq3_ref = I_r*ddtheta_r - T_wr_to_r + T_r_to_b - m_r*g*l_leg_r_com*sin(theta_r + theta_r0) ...
        - m_wr*a_wr_h*l_r*cos(theta_r) - (T_wr_to_r + I_wr*ddtheta_wr)*l_r*cos(theta_r)/R ...
        - (m_b*a_b_v + m_l*a_l_v + m_r*a_r_v)*l_r*sin(theta_r)/2 ...
        - (m_wr - m_b - m_l - m_r - m_wl)*g*l_r*sin(theta_r)/2;
...
disp(eq4);
fprintf('\n推导文档 §3.4 参考形式:\n');
fprintf('  I_l*ddtheta_l = T_wl - T_l + m_l*g*l_leg_l_com*sin(theta_l + theta_l0)\n');
...
% 构建参考方程
eq4_ref = I_l*ddtheta_l - T_wl_to_l + T_l_to_b - m_l*g*l_leg_l_com*sin(theta_l + theta_l0) ...
        - m_wl*a_wl_h*l_l*cos(theta_l) - (T_wl_to_l + I_wl*ddtheta_wl)*l_l*cos(theta_l)/R ...
        - (m_b*a_b_v + m_l*a_l_v + m_r*a_r_v)*l_l*sin(theta_l)/2 ...
        - (m_wl - m_b - m_l - m_r - m_wr)*g*l_l*sin(theta_l)/2;
...
eq6_ref = I_t*ddtheta_t + T_t_to_b - m_t*g*l_tail_com*cos(alpha_t);
